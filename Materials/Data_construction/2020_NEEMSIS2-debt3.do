...
*** Order
order HHID2020 INDID2020 loan_database loanamount  
sort HHID2020 INDID2020 loan_database 
...
********** Creation
gen dummyml=0
replace dummyml=1 if lenderfirsttime!=. & loan_database=="FINANCE"
ta dummyml
gen corr=0
...
ta dummyinterest dummyml


********** Consistency loanamount
sort loanamount
order HHID2020 INDID2020 loanid
*br HHID2020 INDID2020 loanid loan_database loansettled lender4 loanreasongiven loanamount2 loanbalance2 totalrepaid2 principalpaid2 interestpaid2 interestfrequency interestloan2 termsofrepayment repayduration1 repayduration2 
...
** 66
replace interestpaid2=. if interestpaid2==66
replace interestloan2=. if interestloan2==66
...
****************************************
* Principal repayment consistency
****************************************
...
*br HHID2020 INDID2020 loanid loansettled loanamount lender4 loanduration_month loanbalance totalrepaid principalpaid test1 termsofrepayment repayduration1 repayduration2 repayduration2_month pb2
...
********** Consistency interestloan
sort interestloan2
*br HHID2020 INDID2020 loanid loansettled loanreasongiven lender4 loanamount2 loanbalance2  loanduration loanduration_month principalpaid2 interestpaid2 interestfrequency interestloan2 if dummyml==1
...
gen test2=.
replace test2=interestpaid2-test1 if interestpaid2!=.
gen pb1=0 if dummyinterest==1
replace pb1=1 if test1!=interestpaid2 & dummyinterest==1
ta pb1
/*
...
*/
replace interestloan_month=interestpaid2/loanduration_month if test2<1000 & test2!=. & test2>-1000
...
sort test2
sort interestloan
*br HHID2020 INDID2020 loanid loansettled loanamount2 lender4 loanduration_month principalpaid2 interestpaid2 test1 test2 interestfrequency interestloan2 interestloan_month if pb1==1
/*
...
********** Solution la moins risqué avec le plus petit sauf pour les interestloan inf à 100 car pb

sort interestloan2
br HHID2020 INDID2020 loanid loansettled loanamount2 lender4 loanduration_month principalpaid2 interestpaid2 test1 test2 interestfrequency interestloan2 interestloan_month if pb1==1


*replace corr=1 if test2>=0 & (interestfrequency==1 | interestfrequency==2 | interestfrequency==3 | interestfrequency==4) & interestpaid2!=. & interestloan2<100 & pb==1
*replace interestloan2=interestpaid2/loanduration_month if test2>=0 & (interestfrequency==1 | interestfrequency==2 | interestfrequency==3 | interestfrequency==4) & interestpaid2!=. & interestloan2<100 & pb==1

replace interestpaid2=interestloan_month*loanduration_month if test2>0 & (interestfrequency==1 | interestfrequency==2 | interestfrequency==3 | interestfrequency==4) & interestloan2!=. & interestloan2>=100 & pb==1 & corr==0

replace interestloan2=interestpaid2/loanduration_month if test2<=0 & (interestfrequency==1 | interestfrequency==2 | interestfrequency==3 | interestfrequency==4) & interestpaid2!=. & corr==0
drop test2 test1 pb1

drop interestloan_month
...
gen month_repaid=.
replace month_repaid=interestpaid2/interestloan2 if loanduration_month<=12 & dummyinterest==1 & repayduration1==2 & interestfrequency==2
*comme remboursement microcredit reguliers:
replace month_repaid=interestpaid2/interestloan2 if loanduration_month>12 & loanduration_month!=. & lender4==8


**** INTEREST WEEKLY REPAYMENT (interestfreq) - LESS THAN ONE YEAR:

replace month_repaid=(interestpaid2/interestloan2)/4 if loanduration_month<=12 & dummyinterest==1 & interestfrequency==1
...
replace month_repaid=interestpaid2/interestloan2 if lender4==8 & loanduration_month>12 & loanduration_month!=. & repayduration1==2
...
sort interestloan2
*br HHID2020 INDID2020 loanid loansettled loanreasongiven lender4 loanamount2 loanbalance2  loanduration loanduration_month principalpaid2 interestpaid2 interestfrequency interestloan2

gen loan_months=.
replace loan_months=loanduration_month  if interestfrequency==1 | interestfrequency==2 | interestfrequency==6 | interestfrequency==7
replace loan_months=month_repaid if lender4==8 & interestfrequency==2
...
*replace yratepaid=interestpaid2*100*12/(loan_months*loanamount2) if interestfrequency==1 | interestfrequency==2 | interestfrequency==6 | interestfrequency==7 | interestfrequency==4

replace yratepaid=(interestpaid2*100/loanamount2)*(12/loan_months) if interestfrequency==1 | interestfrequency==2 | interestfrequency==6 | interestfrequency==7 | interestfrequency==4

****if interest paid yearly: interestpaid averaged with an integer for number of years 
replace yratepaid=interestpaid2*100/(loan_year2*loanamount2) if interestfrequency==3
...
** For others loans
replace yratepaid=interestpaid2*100/loanamount2 if loanduration<=365 & yratepaid==. & interestpaid2!=.
gen _yratepaid=interestpaid2*365/loanduration if loanduration>365
gen _loanamount=loanamount2*365/loanduration if loanduration>365
replace yratepaid=_yratepaid*100/_loanamount if loanduration>365 & yratepaid==. & interestpaid2!=.

ta dummyinterest
tabstat yratepaid if interestpaid2>0 & interestpaid2!=., by(lender4) stat(n mean p50 min max)

drop yratepaid _loanamount _yratepaid
...
----------------------------------------------------------------
     ...
     lender4 |         N      mean       p50       min       max
-------------+--------------------------------------------------
         WKP |       158  23.22896  21.32469  .9501144  85.82253
   Relatives |        31  18.96839  17.69052       .48  77.78808
      Labour |        74  17.34502  10.00808       .75  109.6812
 Pawn broker |         1        12        12        12        12
Moneylenders |        39  13.18601        10  .2618182   76.1675
     Friends |       304  23.36229  15.32217  .1384864       360
 Microcredit |        65  9.827557  8.774496  .0919027        36
        Bank |        38  16.47736  6.014704     .0105      77.5
-------------+--------------------------------------------------
       Total |       710  20.33105  14.06169     .0105       360
----------------------------------------------------------------
*/
...
*br HHID2020 INDID2020 loanid loansettled loanreasongiven lender4 loanamount2 loanbalance2  loanduration loanduration_month principalpaid2 interestpaid2 interestfrequency interestloan2 yratepaid monthlyinterestrate

tabstat monthlyinterestrate, stat(n mean cv p50 min max) by(lender4)


save "_temp\NEEMSIS2-loans_v16.dta", replace
****************************************
* END
